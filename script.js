...
function calculateTotal() {
  ...
  return {
    ...
    riceCost,
    extrasCost,
    total: mandiCost + riceCost + extrasCost
  };
}
...
/* ============================================================
   ...
   ============================================================ */
function updateUI() {
  const { mandiCost, riceCost, extrasCost, total } = calculateTotal();
  ...
  // Summary lines
  updateSummary({ mandiCost, riceCost, extrasCost, total });
}
...
function updateSummary({ mandiCost, riceCost, extrasCost, total }) {
...
function generateWhatsAppMessage(orderId, custName, custPhone, custAddress, custNote) {
  ...
  let orderLines = [];
  orderLines.push(`• Mandi: ${state.mandi.size === 'half' ? 'Half' : 'Full'} × ${state.mandi.qty} = PKR ${formatPKR(state.mandi.prices[state.mandi.size] * state.mandi.qty)}`);

  if (state.rice.size !== 'none') {
    orderLines.push(`• Rice: ${state.rice.size === 'half' ? 'Half' : 'Full'} × ${state.rice.qty} = PKR ${formatPKR(state.rice.prices[state.rice.size] * state.rice.qty)}`);
  }

  for (const key in state.extras) {
    const ex = state.extras[key];
    if (ex.enabled) {
      orderLines.push(`• ${extraNames[key]}: × ${ex.qty} = PKR ${formatPKR(ex.price * ex.qty)}`);
    }
  }
...
function handleOrderSubmit(e) {
  ...
  const waURL = `https://wa.me/923003434436?text=${encodedMsg}`;
